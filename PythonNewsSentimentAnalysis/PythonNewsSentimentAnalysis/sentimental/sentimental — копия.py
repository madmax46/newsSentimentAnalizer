...
class Sentimental(object):
    def __init__(self, word_list=None, negation=None):
        if word_list is None and negation is None:
            ...
            word_list = [os.path.join(base_dir, p) for p in ['./word_list/emo_dict_ANSI.csv']]
    ...
    def load_word_list(self, filename):
        with open(filename, 'r') as f:
            ...
            for row in reader:
                ...
                tokenizedWord = list(tokenize(word))
                normForms = list()
                for oneW in tokenizedWord:
                    normForms.append(morph.normal_forms(oneW.text)[0])
                word_list[word] = {'score': score,
                             'normForms': normForms
                             }
